 ...
 * Server URL resolution: ai-config.json > localStorage > localhost fallback.
 */

let AI_BASE = 'http://localhost:3001';
let aiChatHistory = [];
let aiChatQuestionIndex = null;

// ── Resolve AI server URL on load ──
(async function initAiBase() {
  try {
    const resp = await fetch('ai-config.json');
    if (resp.ok) {
      const cfg = await resp.json();
      if (cfg.server) { AI_BASE = cfg.server.replace(/\/+$/, ''); return; }
    }
  } catch {}
  const saved = localStorage.getItem('mcq_ai_server');
  if (saved) { AI_BASE = saved.replace(/\/+$/, ''); }
})();
...
async function sendAiChat() {
  try {
    ...
    removeLoading();
    if (!resp.ok) {
      appendChatBubble('ai', '⚠️ AI 服务不可用，请检查后端配置。');
      return;
    }
    ...
    removeLoading();
    appendChatBubble('ai', '⚠️ 无法连接 AI 服务（' + AI_BASE + '）');
  }
}
...
async function requestAnalysis() {
  ...
  try {
    ...
    if (!resp.ok) {
      $('#analysisContent').innerHTML = '<div class="analysis-error">⚠️ AI 服务不可用，请检查后端配置。</div>';
      return;
    }
    const data = await resp.json();
    $('#analysisContent').innerHTML = `<div class="analysis-text">${mdToHtml(data.analysis || '（无分析结果）')}</div>`;
  } catch (e) {
    $('#analysisContent').innerHTML = `<div class="analysis-error">⚠️ 无法连接 AI 服务（${AI_BASE}）</div>`;
  }
}
...
// ═══════ AI Server Config (manual override) ═══════

function configureAiServer() {
  const current = AI_BASE || 'http://localhost:3001';
  const url = prompt('AI 后端地址：', current);
  if (url !== null) {
    const cleaned = url.trim().replace(/\/+$/, '');
    localStorage.setItem('mcq_ai_server', cleaned);
    AI_BASE = cleaned;
    alert('已更新 AI 服务地址为：' + cleaned);
  }
}
